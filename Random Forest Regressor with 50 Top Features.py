...
class RF_regression:
    ...
    def text_sparse(self, X_train, X_valid, X_test, ngram_range=(1, 2)):
        ...
        with open(fileName+'_vectorizer.pickle', 'wb') as handle:
            pickle.dump(tfidf_vectorizer.fit, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def rf_mix(self, X_train, y_train, X_valid, y_valid, n_estimators=10):
        ...
        print('new model')
        clf_mix_new_model = RandomForestRegressor(n_estimators)
        feature_importance = clf_mix.feature_importances_
        indices=np.argsort(feature_importance)[::-1][0:50]

        new_train_dataset=x_mix_train.tocsc()[:, indices]
        new_valid_dataset=x_mix_valid.tocsc()[:, indices]


        clf_mix_new_model.fit(new_train_dataset, y_train['num_lable'])
        # x_arr2matrix_valid = np.array(X_valid['lg_rt_features'].tolist())

        y_predict_mix = clf_mix_new_model.predict(new_valid_dataset)

        mse_mix_new = mean_squared_error(y_valid['num_lable'], y_predict_mix)
        mae_mix_new = mean_absolute_error(y_valid['num_lable'], y_predict_mix)
        mdn_ae_mix_new = median_absolute_error(y_valid['num_lable'], y_predict_mix)
        r2_mix_new = r2_score(y_valid['num_lable'], y_predict_mix)

        print("New mse_mix is: ", mse_mix_new)
        print("New mae_mix is: ", mae_mix_new)
        print("New mdn_ae_mix: ", mdn_ae_mix_new)
        print("New r2_mix: ", r2_mix_new)
...
if __name__ == "__main__":
    ...
    for file_path in glob.glob(data_path + type_rf_):
        ...
        pickle.dump( model_text, open( fileName+"save.p", "wb" ) )

        only_price, model_price = rf.rf_price(rf.X_train, rf.y_train, rf.X_valid, rf.y_valid, n_estimators)
        pickle.dump( model_price, open( fileName+"save.p", "wb" ) )
        ...
        write_file('output_' + output_filename, fileName, [mix_price_text, str(rf.size)], [model_mix])
